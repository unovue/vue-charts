 ...
 * @param [params.labelGapWithTick] - The gap between the label and the tick.
 * @param [params.tickSize] - The length of the tick line.
 * @param [params.tickMargin] - The margin between the tick line and the tick text.
 * @returns The calculated width of the Y-axis.
 */
export function getCalculatedYAxisWidth({
  ticks,
  label,
  ...
  if (ticks) {
    ...
    const tickWidth = (tickSize ?? 0) + (tickMargin ?? 0)
